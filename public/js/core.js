...
var shop = {
  ...
  shop.baseUrl = function admin_link(router) {
    if (typeof router === undefined) {
        router = '';
    }
    router = '/' + router.replace('//', '/');
  ...
  shop.ajax_popup = function(url, method, param, callback, dataType, option) {
      var opt = {
              ...
              baseUrl: (shop.is_obj(option) && shop.is_exists(option.baseUrl)) ? option.baseUrl : shop.baseUrl('')
          };
  ...
  _POST_FORM = function (formId, url, options = {}) {
