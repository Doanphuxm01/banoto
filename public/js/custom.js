    ...
    switch (res.status) {
        ...
        case 'notif-success': {
            $.toast({
                ...
                loader: true,        // Change it to false to disable loader
                loaderBg: '#4CAF50',
                bgColor : '#2dce89', // To change the background
            });
            break;
        }
        case 1: {
...
function _POST_FORM(formId, url, options = {}) {
    ...
    if(typeof formId!=="undefined") {
        ...
        $(formId).find('input:checkbox').map(function() {
            data.push({ name: this.name, value: this.checked ? this.value : "false" }); 
        });
    }else{
...
function _UPLOAD_INIT_LOGO(picker = 'pickfiles', container = '#documentFileRegion', ) {
    ...
    var uploader = new plupload.Uploader({
        ...
        filters: {
            max_file_size: '30mb',
            mime_types: [
                {title: "Chọn 1 ảnh", extensions: "jpg,gif,png"},
            ]
        },
        multipart_params: {'_token': jQuery('meta[name=_token]').attr("content")},
        init: {
            FilesAdded: function (up, files) {
                ...
                $container.find('.js-document-loading').removeClass('hide');
                uploader.start();
            },
            FileUploaded: function (up, file, response) {
                uploader.removeFile(file);
                ...
                let $container = $(container);
                try{
                    $container.find('#partner-logo-image-tag').attr('src',response.data.full_size_link);
                    $container.find('input[type=hidden][name="logo[link]"]').val(response.data.full_size_link);
                    $container.find('input[type=hidden][name="logo[relative_link]"]').val(response.data.relative_link);
                    $container.find('input[type=hidden][name="logo[id]"]').val(response.data.id)
                }catch(err){
                    console.log(err)
                }
...
function select2Ajax(){
    let default_url = '/category/suggest'
}
